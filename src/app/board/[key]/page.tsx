...
export default function BoardPage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f0f1a] p-4">
        <div className="bg-[#1a1a2e] border border-red-500/30 rounded-xl p-8 max-w-md text-center">
          ...
          <p className="text-[#8888a0] text-sm mb-6">{roomError}</p>
          <button
            onClick={() => router.push("/")}
            className="px-6 py-3 bg-[#6c5ce7] text-white rounded-xl hover:bg-[#5a4bd1] transition-all duration-200"
          >
            Go Back Home
          </button>
        </div>
  ...
  if (authEnabled && authLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f0f1a]">
        <div className="w-12 h-12 border-2 border-[#6c5ce7] border-t-transparent rounded-full animate-spin" />
        <p className="text-[#8888a0] text-sm mt-4">Authenticating...</p>
      </div>
    );
  }
  ...
  if (authEnabled && !authUser) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f0f1a] p-4">
        <div className="bg-[#1a1a2e] border border-[#2d2d4a] rounded-xl p-8 max-w-md text-center">
          <h2 className="text-xl font-semibold text-white mb-2">Sign in to join this board</h2>
          <p className="text-[#8888a0] text-sm mb-6">Room key: <span className="text-white font-mono">{roomKey}</span></p>
          <button
            onClick={async () => {
              setSignInError(null);
              try { await signInWithGoogle(); } catch (err) { setSignInError(getFriendlyAuthError(err)); }
            }}
            className="w-full py-3 px-4 bg-white text-gray-800 rounded-xl font-medium text-sm hover:bg-gray-100 transition-all flex items-center justify-center gap-2"
          >
          ...
          <button onClick={() => router.push("/")} className="mt-3 block w-full text-center text-[#8888a0] hover:text-white text-xs">Back to home</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f0f1a] p-4">
        <div className="bg-[#1a1a2e] border border-[#2d2d4a] rounded-xl p-8 max-w-md text-center">
          ...
          <p className="text-[#8888a0] text-sm mb-6">{authError}</p>
          <button
            onClick={async () => {
              try {
                const token = await getIdToken();
            ...
            className="w-full py-3 px-4 bg-[#6c5ce7] text-white rounded-xl font-medium text-sm hover:bg-[#5a4bd1] transition-all"
          >
            Try again
          </button>
          <button onClick={() => router.push("/")} className="mt-3 text-[#8888a0] hover:text-white text-xs">Back to home</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f0f1a]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-2 border-[#6c5ce7] border-t-transparent rounded-full animate-spin" />
          <p className="text-[#8888a0] text-sm">Joining room <span className="text-white font-mono">{roomKey}</span>...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0f0f1a] overflow-hidden">
      {/* Top Bar */}
      <header className="flex items-center justify-between px-4 py-3 bg-[#1a1a2e] border-b border-[#2d2d4a] shrink-0">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push("/")}
            className="p-2 rounded-lg hover:bg-[#252542] text-[#8888a0] hover:text-white transition-all"
          >
          ...
          <div>
            ...
            <p className="text-[#8888a0] text-[10px]">{roomKey}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#252542] hover:bg-[#2d2d4a] text-[#8888a0] hover:text-white text-xs transition-all"
          >
            {copied ? (
              <>
          ...
          <div className="flex -space-x-2">
            {users.slice(0, 5).map((user, i) => (
              <div
                key={user.id}
                className="w-7 h-7 rounded-full bg-gradient-to-br from-[#6c5ce7] to-[#a29bfe] flex items-center justify-center text-[10px] font-bold text-white border-2 border-[#1a1a2e] overflow-hidden"
                title={user.name}
              >
                {user.photoURL ? (
            ...
            {users.length > 5 && (
              <div className="w-7 h-7 rounded-full bg-[#252542] flex items-center justify-center text-[10px] text-[#8888a0] border-2 border-[#1a1a2e]">
                +{users.length - 5}
              </div>
            )}
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Media Type Filter */}
          <div className="flex items-center gap-1 px-4 py-2 bg-[#1a1a2e]/50 border-b border-[#2d2d4a] shrink-0">
            {[
              { type: "all" as const, label: "All", icon: "📋" },
              { type: "image" as const, label: "Photos", icon: "🖼️" },
              { type: "video" as const, label: "Videos", icon: "🎥" },
              ...
              >
                ...
                <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[9px] ${
                  activeMediaType === type ? "bg-white/20" : "bg-[#2d2d4a]"
                }`}>
                  {getMediaCount(type)}
                </span>
              </button>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#6c5ce7] text-white text-xs font-medium hover:bg-[#5a4bd1] transition-all"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* Freeform Board */}
          <div className="relative flex-1 overflow-hidden bg-[#0f0f1a]">
            {filteredMedia.length === 0 ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <div className="w-20 h-20 rounded-2xl bg-[#1a1a2e] border border-[#2d2d4a] flex items-center justify-center mb-4">
                  <svg className="w-10 h-10 text-[#2d2d4a]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.41a2.25 2.25 0 013.182 0l2.909 2.91m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                  </svg>
                </div>
                <h3 className="text-white font-medium mb-1">No media yet</h3>
                <p className="text-[#8888a0] text-xs max-w-xs">
                  {activeMediaType === "all"
                ...
                {activeMediaType === "all" && (
                  <button
                    ...
                    className="mt-4 px-4 py-2 bg-[#6c5ce7] text-white rounded-lg text-xs font-medium hover:bg-[#5a4bd1] transition-all"
                  >
                    Upload Media
                  </button>
                )}
        ...
        {/* Sidebar (Users + Chat) */}
        {showSidebar && (
          <aside className="w-80 border-l border-[#2d2d4a] bg-[#1a1a2e] flex flex-col shrink-0">
            <UserList users={users} />
            <div className="border-t border-[#2d2d4a] flex-1 flex flex-col overflow-hidden">
              <ChatPanel
...
function MediaCard({
  ...
  return (
    <div
      ref={cardRef}
      className={`absolute overflow-hidden bg-black group transition-shadow ${
        dragging
          ? "cursor-grabbing shadow-2xl ring-2 ring-[#6c5ce7]/70"
          : "cursor-grab hover:ring-1 hover:ring-[#6c5ce7]/40"
      }`}
      style={{
        left: `${pos.x}%`,
        top: `${pos.y}%`,
    ...
    >
      ...
      {media.type === "audio" && (
        <div className="w-full flex items-center gap-2 p-3 cursor-pointer">
          ...
          >
            <div
              className="absolute inset-y-0 left-0 bg-[#6c5ce7]"
      ...
      <button
        ...
        className="absolute top-1.5 right-1.5 p-1 rounded-md bg-black/60 text-white/90 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
        title="Delete"
        aria-label="Delete"
      >
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
